      ...
      {wordData.map(({ word, index, isBracketed }) => {
        ...
        return (
          <div
            key={index}
            className={cn(
              "px-4 py-2 rounded-full transition-all duration-300 w-fit flex items-center gap-2",
              ...
              !isCurrent && !isIncorrect && !isHesitated && !isSpoken && isBracketed && "bg-muted/20 text-foreground/60",
              !isCurrent && !isIncorrect && !isHesitated && !isSpoken && !isBracketed && "bg-muted/10 text-foreground/80"
            )}
          >
            {isBracketed && (
